...
import { Grilla } from 'src/app/clases/miJuego/grilla';
import { Caja } from 'src/app/clases/miJuego/caja';

import { Estado } from 'src/app/clases/miJuego/estado';
...
export class MijuegoComponent implements OnInit {
  ...
  constructor(private listado: ListadoService) {
    ...
    this.contenido = [, 1, 2, 3, 4, 5, 6, 7, 8];
    this.boton = "Jugar";
    this.start = "visible";
  ...
  handleClickBox(index: any) {
    ...
    let blankBox = nextdoorBoxes.find(
      nextdoorBox => this.estado.grid[nextdoorBox!.y][nextdoorBox!.x] === 0
    );
    if (blankBox) {
  ...
  render() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      for (let j = 0; j < 3; j++) {
        ...
        this.contenido[this.contador] = this.estado.grid[i][j] === 0 ? "" : this.estado.grid[i][j].toString();
        this.contador++;
      }
